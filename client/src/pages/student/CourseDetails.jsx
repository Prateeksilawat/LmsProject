...
const CourseDetails = () => {
  ...
  const {allCourses , calculateRating, calculateChapterTime,
    calculateNoOfLectures,calculateCourseDuration,currency,backendUrl,userData,getToken} 
  ...
  useEffect(()=>{
     if (userData && Array.isArray(userData.enrolledCourses) && courseData && courseData._id) {
  ...
  return courseData ? (
    ...
    justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
       ...
       <div className='a absolute top-0 left-0 w-full h-[500px] 
          ...
          <p className='pt-4 md:text-base text-sm' 
           ...
           {/* review and ratings */}
            <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                    ...
                    <div className='flex'>
                      {[...Array(5)].map((_,i)=>(<img key = {i} src={i < Math.floor( calculateRating(courseData)) ? assets.star : assets.star_blank} alt='star' 
             ...
             <p className='text-sm'>Course by <span className='text-blue-600 underline'>MERN Stack</span></p>
                      ...
                      courseData.courseContent.map((chapter,index)=>(
                        <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                            ...
                            <div className={`overflow-hidden transition-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'} `}>
                              <ul className='l list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
       ...
       <div className='max-w-[424px] shadow-[0px_4px_15px_2px_rgba(0,0,0,0.1)] z-10 rounded-t md:rounded-none
        ...
        playerData ? 
          <YouTube videoId={playerData.videoId} opts={{playerVars: 
            ...
            <div className='p-5'>
               ...
               <div className='flex items-center text-sm md:text-sm gap-4 pt-2
               md:pt-4 text-gray-500'>
                    ...
                    <div className='flex items-center gap-1'>
                      <img src={assets.time_clock_icon} alt='clockIcon'/>
                    ...
                    <div className='flex items-center gap-1'>
                      <img src={assets.lesson_icon} alt='lessonkIcon'/>
                      <p>{calculateNoOfLectures(courseData)} lesson</p>
                    </div>

               </div>
               <button onClick={enrollCourse} className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 cursor-pointer
               text-white font-medium'
               >{isAlreadyEnrolled ? 'Already Enrolled' : 'Enroll Now'}</button>

               <div className='pt-6'>
                <p className='md:text-xl text-lg font-medium text-gray-800'>What's in the course ?</p>
                <ul className='ml-4 pt-2 text-sm md:text-sm list-disc text-gray-500'>
